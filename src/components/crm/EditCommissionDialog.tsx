...
const EditCommissionDialog = ({ commission, open, onOpenChange, sellers, clients, onSuccess }: EditCommissionDialogProps) => {
  ...
  useEffect(() => {
    if (commission) {
      setSellerId(commission.seller_id || '');
      setClientId(commission.client_id || '');
      setIsActive(commission.is_active || true);
      setCommissionTiers(commission.commission_tiers || [{ min_amount: 0, max_amount: 1000, commission_rate: 3 }]);
    }
  }, [commission]);
  ...
  const handleSubmit = async () => {
    ...
    setIsLoading(true);
    try {
      await crmService.updateSellerCommission(commission.id, {
        seller_id: sellerId,
        client_id: clientId,
        ...
        <div className="space-y-6">
          ...
          <div className="flex justify-end space-x-2">
            ...
            <Button onClick={handleSubmit} disabled={isLoading}>
              {isLoading ? 'Updating...' : 'Update Commission Structure'}
            </Button>
          </div>
        </div>
      </DialogContent>
